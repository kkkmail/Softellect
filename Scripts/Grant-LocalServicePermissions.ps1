function Grant-LocalServicePermissions {
    [CmdletBinding()]
    param (
    ...
    # This function grants the LOCAL SERVICE account the necessary permissions
    ...
    try {
        ...
        Write-ServiceLog -Message "Successfully granted LOCAL SERVICE access to service configuration"
        ...
        if ($currentSddl) {
            ...
            if ($currentSddl -notlike "*$sid*") {
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-ServiceLog -Message "Successfully granted service control manager permissions to LOCAL SERVICE"
                } else {
                    Write-ServiceLog -Message "Failed to set SCM permissions. Exit code: $LASTEXITCODE" -Level "Warning"
                }
            } else {
                Write-ServiceLog -Message "LOCAL SERVICE already has SCM permissions"
            }
        }

    } catch {
        Write-ServiceLog -Message "Failed to grant access to service configuration: $_" -Level "Warning"
    }
    ...
    if (Test-Path -Path $FolderPath) {
        try {
            ...
            Write-ServiceLog -Message "Successfully granted LOCAL SERVICE account permissions to $FolderPath"
        } catch {
            Write-ServiceLog -Message "Failed to grant folder permissions: $_" -Level "Warning"
        }
    } else {
        Write-ServiceLog -Message "Folder path does not exist: $FolderPath" -Level "Error"
    }
    ...
    foreach ($groupName in $groups) {
        try {
            ...
            try {
                ...
                Write-ServiceLog -Message "Successfully added LOCAL SERVICE to $groupName group"
            } catch {
                if ($_.Exception.Message -like "*already a member*") {
                    Write-ServiceLog -Message "LOCAL SERVICE is already a member of $groupName group"
                } else {
                    ...
                    if (Get-LocalGroup -Name $groupName -ErrorAction SilentlyContinue) {
                        ...
                        Write-ServiceLog -Message "Group $groupName does not exist on this system"
                    }
                }
            }
        } catch {
            Write-ServiceLog -Message "Could not add LOCAL SERVICE to $groupName group: $_" -Level "Warning"
        }
    }
...
"@
        ...
        if ($result.ExitCode -eq 0) {
            Write-ServiceLog -Message "Successfully granted service logon right to LOCAL SERVICE"
        } else {
            Write-ServiceLog -Message "Service logon right grant completed with code $($result.ExitCode)"
        }
        ...
        Write-ServiceLog -Message "Failed to grant service logon right: $_" -Level "Warning"
    }

    Write-ServiceLog -Message "Permission granting process completed"
}
