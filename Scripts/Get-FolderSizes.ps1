function Get-FolderSizes {
   ...
   foreach ($topFolder in $topLevelFolders) {
       ...
       try {
           # Test if we can actually access the folder before proceeding
           $testAccess = Get-ChildItem -Path $topFolder.FullName -ErrorAction Stop -Force | Select-Object -First 1
           
           # Recursively get all subfolders for the current top-level folder
           $subFolders = Get-ChildItem -Path $topFolder.FullName -Directory -Recurse -Force -ErrorAction SilentlyContinue
           
           # Create array with top-level folder and all subfolders
           $folders = @($topFolder) + @($subFolders)
                   ...
                   foreach ($file in $files) {
                       ...
                       if ($textFileTypesLower -contains $extension) {
                           try {
                               ...
                               Write-ServiceLog -Message "Could not read file for LOC: $($file.FullName) - Error: $($_.Exception.Message)" -Level "Warning"
                           }
                       }
                   }
               } catch {
                   # Collect inaccessible folders and report error with details
                   Write-ServiceLog -Message "Could not access folder: $($folder.FullName) - Error: $($_.Exception.Message)" -Level "Error"
                   $inaccessibleFolders += $folder.FullName
               }
           }
       } catch {
           # Collect inaccessible top-level folders and report error with details
           Write-ServiceLog -Message "Could not access top-level folder: $($topFolder.FullName) - Error: $($_.Exception.Message)" -Level "Error"
