   ...
   foreach ($topFolder in $topLevelFolders) {
       ...
       try {
           ...
           foreach ($folder in $folders) {
               ...
               try {
                   # Get the total size of all files in the folder (excluding subfolders) matching the file mask
                   $size = (Get-ChildItem -Path $folder.FullName -File -Filter $FileMask -Force -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum
