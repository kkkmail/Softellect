                ...
                if dstIp <> externalIp then
                    ...
                    | Tcp | Udp ->
                        ...
                        | true, entry ->
                            ...
                            Logger.logTrace (fun () ->
                                $"NAT IN: proto={proto}, extPort={dstPort} -> {internalIp:X8}:{internalPortOrId}")

                            Some packet
                        | false, _ ->
                            // No mapping – drop
                            Logger.logTrace (fun () ->
                                $"NAT IN: proto={proto}, no mapping for extPort={dstPort}, dropping packet")
                            None
                    ...
                    | Icmp ->
                        ...
                        if icmpType = 0uy then
                            ...
                            | true, entry ->
                                ...
                                Logger.logTrace (fun () ->
                                    $"NAT IN: proto=Icmp, extId={identifier} -> {internalIp:X8}:id={internalId}")

                                Some packet
                            | false, _ ->
                                // No mapping – drop
                                Logger.logTrace (fun () ->
                                    $"NAT IN: proto=Icmp, no mapping for extId={identifier}, dropping packet")
                                None
                        else
                            // Non-echo-reply ICMP – drop (no mapping possible)
                            Logger.logTrace (fun () ->
                                $"NAT IN: proto=Icmp, type={icmpType} not echo reply, dropping packet")
                            None
