...
module Implementation =
    ...
    type ClientAdmContext =
        ...
        static member create() =
            {
                clientAccessInfo = loadVpnClientAccessInfo tryDetectPhysicalNetwork
            }
    ...
    let detectPhysicalNetwork (_ctx: ClientAdmContext) =
        ...
        | Ok i ->
            Logger.logInfo $"Writing to appsettings.json: PhysicalGatewayIp = '%A{i.gatewayIp}', PhysicalInterfaceName = '{i.interfaceName}'."

            match tryWritePhysicalNetworkConfig i.gatewayIp i.interfaceName with
            | Ok () ->
                Logger.logInfo "Physical network configuration saved successfully."
                Ok $"Detected and saved: PhysicalGatewayIp = '%A{i.gatewayIp}', PhysicalInterfaceName = '{i.interfaceName}'."
            | Error e ->
