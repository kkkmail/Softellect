...
module Service =
    ...
    type MessagingServiceData<'M, 'E> =
        {
            messagingServiceProxy : MessagingServiceProxy<'M, 'E>
    ...
    type MessagingService<'M, 'E>(d : MessagingServiceData<'M, 'E>) =
        let proxy = d.messagingServiceProxy
        member _.getVersion() : StlResult<MessagingDataVersion, 'E> = Ok d.messagingDataVersion

        member _.sendMessage (m : Message<'M>) : UnitResult<'E> = proxy.saveMessage m
        member _.tryPeekMessage (n : MessagingClientId) : StlResult<Message<'M> option, 'E> = proxy.tryPickMessage n
    ...
    let createMessagingServiceEventHandlers logger (w : MessagingService<'M, 'E>) =
