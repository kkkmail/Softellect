...
function Uninstall-Service {
    ...
    try {
        ...
        try {
            ...
            # Check exit code in case script uses Exit instead of Throw
            if ($LASTEXITCODE -ne 0) {
                Write-Log -Level Error -Message "Uninstall script failed with exit code: $LASTEXITCODE"
...
function Install-Service {
    ...
    try {
        ...
        try {
            ...
            # Check exit code in case script uses Exit instead of Throw
            if ($LASTEXITCODE -ne 0) {
                Write-Log -Level Error -Message "Install script failed with exit code: $LASTEXITCODE"
